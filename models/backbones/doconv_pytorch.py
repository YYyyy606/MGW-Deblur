...
class DOConv2d(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, D_mul=None, stride=1,
        ...
        # init.kaiming_uniform_(self.W, a=math.sqrt(5))  #原文的权重初始化
        init.kaiming_uniform_(self.W, a=0,mode="fan_in")
        self.W.data*=0.1
        ...
        if M * N > 1:
            ...
            if self.D_mul % (M * N) != 0:  # the cases when D_mul > M * N
                ...
                self.D_diag = Parameter(torch.cat([D_diag, zeros], dim=2), requires_grad=True)
            else:  # the case when D_mul = M * N
                self.D_diag = Parameter(D_diag, requires_grad=True)
        ##################################################################################################
        if simam:
