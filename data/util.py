...
def _get_paths_from_images(path):
    ...
    assert os.path.isdir(path), "{:s} is not a valid directory".format(path)  #判断路径是否是有效目录
    images = []
    for dirpath, _, fnames in sorted(os.walk(path)):   #迭代读取文件路径, dirpath当前文件夹路径,fnames文件名
...
def _get_paths_from_lmdb(dataroot):
    ...
    paths = meta_info["keys"]   #图片名 000_00000000
    sizes = meta_info["resolution"]  #3_720_1280 单张图片的shape
    if len(sizes) == 1:
